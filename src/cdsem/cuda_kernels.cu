...
__global__ void __init_trajectory(cuda_particle_struct pstruct, cuda_geometry_struct gstruct, cuda_material_struct mstruct, curandState* rand_state_dev_p) {
    ...
    int status = pstruct.status_dev_p[particle_idx];
    switch(status) {
    ...
    float distance = 2.0*norm3df(gstruct.AABB_halfsize.x, gstruct.AABB_halfsize.y, gstruct.AABB_halfsize.z);
    ...
    if(material_idx >= 0) {
        if(pstruct.K_energy_dev_p[particle_idx] < mstruct.barrier_dev_p[material_idx]) {
            ...
            const float kinetic_energy = pstruct.K_energy_dev_p[particle_idx];
            ...
            const float x = __fdividef(__logf(kinetic_energy/K_min), __logf(K_max/K_min))*(K_cnt-1);
            const int ix = clamp(__float2int_rd(x), 0, K_cnt-2);

            const float elastic_imfp =
                __expf(interp1(mstruct.elastic_dev_p, mstruct.table_pitch, P_cnt+1, ix, 0, material_idx, x-ix));
            const float inelastic_imfp =
                __expf(interp1(mstruct.inelastic_dev_p, mstruct.table_pitch, P_cnt+1, ix, 0, material_idx, x-ix));
    ...
    pstruct.status_dev_p[particle_idx] = status;
    pstruct.distance_dev_p[particle_idx] = distance;
}
...
__global__ void __update_trajectory(cuda_particle_struct pstruct, cuda_geometry_struct gstruct, cuda_material_struct mstruct) {
    ...
    do {
        ...
        for(int i = 20-__clzll(location)/3; i >= 0; i--) {
            const unsigned int octant = (location>>(i*3))&7;
            index = cuda_make_ptr(gstruct.octree_dev_p, gstruct.octree_pitch, index)[octant];
            AABB.x += AABB.w*gstruct.AABB_halfsize.x*((octant&1)-0.5f);
            AABB.y += AABB.w*gstruct.AABB_halfsize.y*(((octant&2)>>1)-0.5f);
            AABB.z += AABB.w*gstruct.AABB_halfsize.z*(((octant&4)>>2)-0.5f);
            AABB.w *= 0.5f;
        }
        ...
        const int* triangles_p = cuda_make_ptr(gstruct.octree_dev_p, gstruct.octree_pitch, -index);
        ...
        for(int i = 0; i < gstruct.occupancy; i++) {
            const int triangle_idx = triangles_p[i];
            if(triangle_idx < 0)
                break;
            ...
            if((material_idx_out == material_idx) || (material_idx_out == triangle::NOP))
                continue;
            else if((material_idx_out == triangle::DETECTOR_LT50) && (kinetic_energy >= 50))
                continue;
            else if((material_idx_out == triangle::DETECTOR_GE50) && (kinetic_energy < 50))
                continue;
...
__global__ void __apply_intersection_event(cuda_particle_struct pstruct, cuda_geometry_struct gstruct, cuda_material_struct mstruct, curandState* rand_state_dev_p) {
    ...
    const float kinetic_energy = pstruct.K_energy_dev_p[particle_idx];
    const float z = sqrtf(1.0f+dU/(kinetic_energy*cos_alpha*cos_alpha));
    if((kinetic_energy*cos_alpha*cos_alpha+dU > 0) && (curand_uniform(&rand_state) < __fdividef(4.0f*z, ((1.0f+z)*(1.0f+z))))) {
        ...
        pstruct.K_energy_dev_p[particle_idx] = kinetic_energy+dU;
        pstruct.material_idx_dev_p[particle_idx] = material_idx_out;
    } else if((dU < 0) && (curand_uniform(&rand_state) < __expf(1.0f+0.5f*kinetic_energy/dU))) {
...
__global__ void __apply_elastic_event(cuda_particle_struct pstruct, cuda_material_struct mstruct, curandState* rand_state_dev_p) {
    ...
    const int material_idx = pstruct.material_idx_dev_p[particle_idx];
    const float cos_theta = clamp(interp2(mstruct.elastic_dev_p, mstruct.table_pitch, P_cnt+1, ix, 1+iy, material_idx, x-ix, y-iy), -1.0f, 1.0f);
...
__global__ void __apply_inelastic_event(cuda_particle_struct pstruct, cuda_material_struct mstruct, curandState* rand_state_dev_p) {
    ...
    if(primary_idx >= pstruct.capacity)
        return;

    int status = pstruct.status_dev_p[primary_idx];
    ...
    const float kinetic_energy = pstruct.K_energy_dev_p[primary_idx];
    const float K_min = mstruct.K_energy_range.x;
    const float K_max = mstruct.K_energy_range.y;
    const int K_cnt = mstruct.table_dim.x;
    const int P_cnt = mstruct.table_dim.y;
    ...
    const float omega0 = interp2(mstruct.inelastic_dev_p, mstruct.table_pitch, P_cnt+1, ix, 1+iy, material_idx, x-ix, y-iy);

    float binding_energy = -1.0f;
    if(omega0 > 100.0f) {
        ...
        binding_energy = cuda_make_ptr(mstruct.ionization_dev_p, mstruct.table_pitch, P_cnt, iy, material_idx)[ix];
        if(binding_energy < 50.0f)
            binding_energy = -1.0f;
    }
    if(binding_energy < 0) {
        if(material_idx == 0) {
            /* silicon */
            if(omega0 > 100.0f)
                binding_energy = 100.0f;
            else if(omega0 > 8.9f)
                binding_energy = 8.9f;
            else if(omega0 > 5.0f)
                binding_energy = 5.0f;
            else if(omega0> 1.12f)
                binding_energy = 1.12f;
    ...
    const float fermi_energy = mstruct.fermi_dev_p[material_idx];
    // upper limit of Eq.9 (Ashley), but corrected for the fermi energy.
    float omega_max = 0.5f*(kinetic_energy+omega0-fermi_energy);
    float omega_min = omega0;
    float w0 = min(omega0-1.0f, fmaxf(0.0f, binding_energy)-fermi_energy);
    if(kinetic_energy > 2.0f*omega0) {
        // Eq. 10 (Ashley)
        omega_min = 0.5f*kinetic_energy*(1.0f-sqrtf(1.0f-2.0f*omega0/kinetic_energy)+omega0/kinetic_energy);
        w0 = omega0;
    }
    float omega;
    if((w0 > 0) && (omega_min > w0) && (omega_min < omega_max)) {
        ...
        omega = __fdividef(w0, (1.0f-(1.0f-w0/omega_min)*__expf(U*log1pf(-w0/omega_max))*__expf(-U*log1pf(-w0/omega_min))));
    } else {
        ...
        omega = __fdividef(omega0, (1.0f-curand_uniform(&rand_state)*(1.0f-omega0/omega_max)));
    }
    ...
    if(binding_energy < 0) {
        ...
        if(bandgap < 0) {
            ...
            */
            ...
            binding_energy = bandgap;
        } else {
            // phonon loss
            pstruct.K_energy_dev_p[primary_idx] = kinetic_energy-omega0;
            rand_state_dev_p[primary_idx] = rand_state;
            return;
        }
    }

    binding_energy = fmaxf(0.0f, binding_energy);
    const float K = kinetic_energy-fermi_energy+2.0f*binding_energy;
    const float dK = binding_energy+omega;
    const float cos_alpha = __saturatef(sqrtf(__fdividef((1.0f-dK/K)*(1.0f+0.5f*K/mc2), 1.0f+0.5f*(K-dK)/mc2)));
    const float sin_alpha = sqrtf(1.0f-cos_alpha*cos_alpha);
    const float cos_beta = __saturatef(sqrtf(__fdividef((dK/K)*(1.0f+0.5f*K/mc2), 1.0f+0.5f*dK/mc2)));
    ...
    pstruct.K_energy_dev_p[primary_idx] = kinetic_energy-omega;
    ...
    pstruct.K_energy_dev_p[secondary_idx] = fermi_energy+omega-binding_energy;
