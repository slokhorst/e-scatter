...
__global__ void __init_trajectory(particle_struct pstruct, geometry_struct gstruct, material_struct mstruct) {
	...
	if(status == particle_struct::TERMINATED)
		return;
	else if(status == particle_struct::DETECTED)
		return;
	else if(status == particle_struct::PENDING)
		return;
	...
	if(gid.w >= 0) {
		...
		gid.x = __float2int_rz((pos.x-gstruct.min.x)/gstruct.delta.x);
		gid.y = __float2int_rz((pos.y-gstruct.min.y)/gstruct.delta.y);
		gid.z = __float2int_rz((pos.z-gstruct.min.z)/gstruct.delta.z);
		if((gid.x < 0) || (gid.y < 0) || (gid.z < 0)) {
			pstruct.status_dev_p[pid] = particle_struct::TERMINATED;
			return;
	...
	float4 isec;
	isec.x = fabsf((gstruct.min.x+(ceilf(0.5f*dir.x)+gid.x)*gstruct.delta.x-pos.x)/dir.x);
	isec.y = fabsf((gstruct.min.y+(ceilf(0.5f*dir.y)+gid.y)*gstruct.delta.y-pos.y)/dir.y);
	isec.z = fabsf((gstruct.min.z+(ceilf(0.5f*dir.z)+gid.z)*gstruct.delta.z-pos.z)/dir.z);
	if((isec.x < isec.y) && (isec.x < isec.z)) {
		status = particle_struct::GRID_X_EVENT;
		isec.w = isec.x;
	} else if((isec.y < isec.x) && (isec.y < isec.z)) {
		status = particle_struct::GRID_Y_EVENT;
		isec.w = isec.y;
	} else {
		status = particle_struct::GRID_Z_EVENT;
		isec.w = isec.z;
	}
	...
	if(mid >= 0)
		if(pstruct.K_dev_p[pid] < mstruct.barrier_dev_p[mid])
			status = particle_struct::TERMINATED;
...
__global__ void __update_trajectory(particle_struct pstruct, geometry_struct gstruct) {
	...
	float3 pos = make_float3(pstruct.rx_dev_p[pid], pstruct.ry_dev_p[pid], pstruct.rz_dev_p[pid]);
	pos.x += dir.x*ds;
	pos.y += dir.y*ds;
	pos.z += dir.z*ds;
	if((pos.x < gstruct.min.x) || (pos.x > gstruct.max.x))
		status = pstruct.status_dev_p[pid] = particle_struct::TERMINATED;
	if((pos.y < gstruct.min.y) || (pos.y > gstruct.max.y))
		status = pstruct.status_dev_p[pid] = particle_struct::TERMINATED;
	if((pos.z < gstruct.min.z) || (pos.z > gstruct.max.z))
		status = pstruct.status_dev_p[pid] = particle_struct::TERMINATED;
	pstruct.rx_dev_p[pid] = clamp(pos.x, gstruct.min.x, gstruct.max.x);
	pstruct.ry_dev_p[pid] = clamp(pos.y, gstruct.min.y, gstruct.max.y);
	pstruct.rz_dev_p[pid] = clamp(pos.z, gstruct.min.z, gstruct.max.z);
	if(status != particle_struct::GRID_X_EVENT)
	if(status != particle_struct::GRID_Y_EVENT)
	if(status != particle_struct::GRID_Z_EVENT)
		return;
	int4 gid;
	gid.w = pstruct.gid_dev_p[pid];
	gid.x = gid.w%gstruct.dim.x;
	...
	if(status == particle_struct::GRID_X_EVENT) {
		...
		if((gid.x < 0) || (gid.x >= gstruct.dim.x))
			pstruct.status_dev_p[pid] = particle_struct::TERMINATED;
	} else if(status == particle_struct::GRID_Y_EVENT) {
		gid.y += copysignf(1.0f, dir.y);
		if((gid.y < 0) || (gid.y >= gstruct.dim.y))
			pstruct.status_dev_p[pid] = particle_struct::TERMINATED;
	} else if(status == particle_struct::GRID_Z_EVENT) {
...
__global__ void __probe_isec_event(particle_struct pstruct, geometry_struct gstruct) {
	...
	if(pid >= pstruct.capacity)
		return;
	int status = pstruct.status_dev_p[pid];
	if(status == particle_struct::DETECTED)
		return;
	else if(status == particle_struct::TERMINATED)
		return;
	else if(status == particle_struct::PENDING)
		return;
	...
	for(int ix = 0; ix < gstruct.dim.w; ix++) {
		...
		if((mid_in == geometry_struct::NOP) && (mid_out == geometry_struct::NOP))
			break;
		float3 T;
		...
		if(dot_product(cross_product(e1, e2), dir) < 0)
			mid_out = mid_in;
		if((mid_out == mid) || (mid_out == geometry_struct::NOP))
			continue;
		else if((mid_out == geometry_struct::DETECTOR_LT50) && (K >= 50))
			continue;
		else if((mid_out == geometry_struct::DETECTOR_GE50) && (K < 50))
			continue;
	...
	if(tid >= 0) {
		pstruct.status_dev_p[pid] = particle_struct::ISEC_EVENT;
...
__global__ void __apply_isec_event(particle_struct pstruct, geometry_struct gstruct, material_struct mstruct, curandState* rand_state_dev_p) {
	...
	int status = pstruct.status_dev_p[pid];
	if(status != particle_struct::ISEC_EVENT)
		return;
	float3 dir = make_float3(pstruct.dx_dev_p[pid], pstruct.dy_dev_p[pid], pstruct.dz_dev_p[pid]);
	dir = dir*rnorm3df(dir.x, dir.y, dir.z);
	...
	switch(mid_out) {
		case geometry_struct::DETECTOR:
		case geometry_struct::DETECTOR_LT50:
		case geometry_struct::DETECTOR_GE50:
			pstruct.status_dev_p[pid] = particle_struct::DETECTED;
			return;
		default:
			break;
	}
...
__global__ void __probe_scatter_event(particle_struct pstruct, material_struct mstruct, curandState* rand_state_dev_p) {
	...
	const int ix = clamp(__float2int_rz(x), 0, mstruct.Kn-2);
	...
	if(attenuation < pstruct.ds_dev_p[pid]) {
		status = particle_struct::ELASTIC_EVENT;
		if(curand_uniform(&rand_state) > elastic_imfp/total_imfp)
			status = particle_struct::INELASTIC_EVENT;
		pstruct.status_dev_p[pid] = status;
...
__global__ void __apply_elastic_event(particle_struct pstruct, material_struct mstruct, curandState* rand_state_dev_p) {
	...
	const int pid = pstruct.pid_dev_p[i];
	int status = pstruct.status_dev_p[pid];
	if(status != particle_struct::ELASTIC_EVENT)
		return;
	curandState rand_state = rand_state_dev_p[pid];
	const float x = __fdividef(__logf(pstruct.K_dev_p[pid]/mstruct.K1), __logf(mstruct.K2/mstruct.K1))*(mstruct.Kn-1);
	const int ix = clamp(__float2int_rz(x), 0, mstruct.Kn-2);
	const float y = curand_uniform(&rand_state)*(mstruct.Pn-1);
	const int iy = clamp(__float2int_rz(y), 0, mstruct.Pn-2);
	const int mid = pstruct.mid_dev_p[pid];
...
__global__ void __apply_inelastic_event(particle_struct pstruct, material_struct mstruct, curandState* rand_state_dev_p) {
	...
	const int pid = pstruct.pid_dev_p[i];
	int status = pstruct.status_dev_p[pid];
	if(status != particle_struct::INELASTIC_EVENT)
	if(status != particle_struct::PENDING)
		return;
	const int sid = pstruct.pid_dev_p[pstruct.capacity-1-i];
	if(pstruct.status_dev_p[sid] != particle_struct::TERMINATED) {
		pstruct.status_dev_p[pid] = particle_struct::PENDING;
		return;
	}
	if(status == particle_struct::PENDING)
		pstruct.status_dev_p[pid] = particle_struct::INELASTIC_EVENT;
	curandState rand_state = rand_state_dev_p[pid];
	const float K = pstruct.K_dev_p[pid];
	float x, y;
	int ix, iy;
	x = __fdividef(__logf(K/mstruct.K1), __logf(mstruct.K2/mstruct.K1))*(mstruct.Kn-1);
	ix = clamp(__float2int_rz(x), 0, mstruct.Kn-2);
	y = curand_uniform(&rand_state)*(mstruct.Pn-1);
	iy = clamp(__float2int_rz(y), 0, mstruct.Pn-2);
	const int mid = pstruct.mid_dev_p[pid];
	...
	if(omega0 > 100.0f) {
		...
		const int iy = __float2int_rz(y);
		B = cuda_make_ptr(mstruct.ionization_dev_p, mstruct.pitch, mstruct.Pn+1, 1+iy, mid)[ix];
		if(B < 50.0f)
			B = -1;
	}
	if(B < 0) {
		const float* binding_p = cuda_make_ptr(mstruct.ionization_dev_p, mstruct.pitch, mstruct.Pn+1, 0, mid);
	...
	pstruct.status_dev_p[sid] = particle_struct::NEW_SECONDARY;
