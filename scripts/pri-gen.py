...
parser = argparse.ArgumentParser(description='Create exposure file containing primary electrons')
parser.add_argument('--dose',   '-D', type=float, default=2000.0, help='dose [uC/cm^2]')
parser.add_argument('--spot',   '-S', type=float, default=1.5,    help='spot size [nm]')
parser.add_argument('--energy', '-K', type=float, default=300.0,  help='energy [eV]')
parser.add_argument('--nx',           type=int,   default=128,    help='x resolution [px]')
...
print('dose: {} [μC/cm^2]'.format(args.dose), file=sys.stderr)
print('      {} [e/px]'.format(nperpx), file=sys.stderr)

for yi in range(0,Y.size):
	print('generating exposure [{}%]'.format(int(100*(yi)/(Y.size-1))), end="\r", file=sys.stderr)
	for xi in range(0,X.size):
		R = numpy.random.multivariate_normal([X[xi], Y[yi]], S*numpy.identity(2), numpy.random.poisson(nperpx))
		for r in R:
			sys.stdout.buffer.write(struct.pack('7f', r[0],r[1],Z,dx,dy,dz,K))
			sys.stdout.buffer.write(struct.pack('2i', xi,yi))
sys.stdout.flush()
