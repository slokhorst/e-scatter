...
def parse_eedl_file(filename):
	...
	with open(filename) as f:
		...
		while ln < len(lines):
			row = parse_endf_line(lines[ln])
			# 23/534--23/599: Electroionization Subshell Cross Sections shell 1--66
...
def parse_folder(dirname, z):
	eadl_files = glob.glob(os.path.join(dirname,"atomic_relax","atom-{0:03d}_*_*.endf".format(z)))
	eedl_files = glob.glob(os.path.join(dirname,"electrons","e-{0:03d}_*_*.endf".format(z)))

	if len(eadl_files) != 1:
		raise FileNotFoundError(os.path.join(dirname,"atomic_relax","atom-{0:03d}_*_*.endf".format(z)))
	if len(eedl_files) != 1:
		raise FileNotFoundError(os.path.join(dirname,"electrons","e-{0:03d}_*_*.endf".format(z)))
	...
	for subi, (ebi,eln) in sorted(parse_eadl_file(eadl_fpath).items()):
		print('<cstable type="ionization" shell="{}" occupancy="{}" binding-energy="{}*eV">'.format(subi, eln, ebi))
		for energy, cs in sorted(shell_cross_sections[subi].items()):
			print('\t<cross-section energy="{}*eV" cs="{}*m^2" />'.format((energy-ebi),cs))
		print('</cstable>\n')
